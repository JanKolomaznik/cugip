...
struct dim_traits
{
	...
	typedef simple_vector<float, tDim> float_coord_t;
...
//TODO better result type deduction
template<typename TType, int tDimension>
inline CUGIP_DECL_HYBRID simple_vector<int, tDimension>
round(const simple_vector<TType, tDimension> &aVector)
{
	simple_vector<int, tDimension> result;
	for (int i = 0; i < tDimension; ++i) {
#ifdef __CUDA_ARCH__
		result[i] = roundf(aVector[i]);
#else
		result[i] = std::round(aVector[i]);
#endif
	}
	return result;
}
...
rotate(const simple_vector<TCoordType, 2> &aVector, double aAngle)
{
	auto sina = sin(aAngle);
	auto cosa = cos(aAngle);

	return simple_vector<TCoordType, 2>(
		cosa * aVector[0] - sina * aVector[1],
		sina * aVector[0] + cosa * aVector[1]
		);
}
